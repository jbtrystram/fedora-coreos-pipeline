...
lock(resource: "build-node-image") {
    ...
    try {
        ...
        def (registry_staging_repo, registry_staging_tags, registry_prod_repo, registry_prod_tags)
                    = pipeutils.get_ocp_node_registry_repo(pipecfg, params.RELEASE, timestamp)

        // staging_tags is a list for consistency with the `prod` opbjects
        // but don't need more than one tag is this is just used to store
        // intermediary images before referencing them in a multi-arch
        // manifest.
        // So let's simply stick the first tag here and ignore any others.
        ...
        if (params.PIPECFG_HOTFIX_REPO || params.PIPECFG_HOTFIX_REF) {
            registry_staging_tag += "-hotfix-${pipecfg.hotfix.name}"
        }
        stage('Build Node Image') {
            withCredentials([file(credentialsId: 'oscontainer-push-registry-secret', variable: 'REGISTRY_AUTH_FILE')]) {
                 ...
                 pipeutils.build_and_push_image(arches: arches,
                                                ...
                                                manifest_tag_staging: "${registry_staging_tag}-manifest",
        ...
        stage('Build Extensions Image') {
            withCredentials([file(credentialsId: 'oscontainer-push-registry-secret', variable: 'REGISTRY_AUTH_FILE')]) {
                ...
                pipeutils.build_and_push_image(arches: arches,
                                               ...
                                               manifest_tag_staging: "${registry_staging_tag}-extensions-manifest",
                                               secret: "id=yumrepos,src=${yumrepos_file}", // notsecret (for secret scanners)
                                               from: build_from,
